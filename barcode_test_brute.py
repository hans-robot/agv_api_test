...
class TcpClient(object):
    ...
    def call(self, request):
        ...
        #print "sent[ %s ]"%('-'.join(map(lambda x:"%02X"%ord(x), request_bytes[:4])) + " | " + repr(request_bytes[4:].replace(" ","").replace("\n",""))[1:-1])
...
if __name__ == '__main__':
    ...
    barcode_client = TcpClient(agv_ip, 8892)
    ...
    #res__ = task_client.call(add_mag_task(3))
    #print res__
    ...
    while(True):
        ...
        #print(cmd__)
        res__ = barcode_client.call(cmd__)
        #task_state_ = json.loads(res__)["data"]["taskState"]
        data__ = json.loads(res__)["data"]
        if(data__):
            print "%s"%' , '.join(data__)
